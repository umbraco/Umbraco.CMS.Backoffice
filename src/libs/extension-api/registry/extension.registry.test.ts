...
describe('Add Conditions', () => {
	...
	it('allows an extension condition to be updated', async () => {
		...
		expect(updatedWorkspaceExt.conditions?.[0]?.alias).to.equal(UMB_WORKSPACE_ALIAS_CONDITION);
	});
	...
	it('allows an extension to update with multiple conditions', async () => {
		...
		expect(extUpdated.conditions?.[2]?.alias).to.equal(UMB_WORKSPACE_ALIAS_CONDITION);
	});
